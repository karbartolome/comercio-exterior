...
for (i in c(1962:1967)) {
  print(i)
  temp=paste0('~/Documents/Analisis/datos_comercio/comercio-exterior/data/yrpc-',i,'.rds')
  df=readRDS(temp)
  ...
  df_section_exports = df %>% 
      ...
      left_join(paises %>% select(reporter_iso=country_iso,
                                  reporter=country_name_english, 
                                  continent_reporter = continent), by='reporter_iso') %>% 
      left_join(paises %>% select(partner_iso=country_iso,
                                  partner=country_name_english, 
                                  continent_partner = continent), by='partner_iso') %>% 
      select(reporter, partner, section=section_shortname_english, export_value_usd, continent_reporter, continent_partner) 
    
  write.csv(df_section_exports, paste0('~/Documents/Analisis/datos_comercio/comercio-exterior/data/df_',i,'.csv'), 
            row.names = FALSE)
...
write.csv(df_section_exports, '/Users/karinabartolome/Documents/Analisis/datos_comercio/comercio-exterior/df.csv', row.names = FALSE)

library(tidyr)
df_spread = df_section_exports %>% spread(key='section',value='export_value_usd')

df_spread = df_spread %>% replace(is.na(.), 0)


write.csv(df_spread, '/Users/karinabartolome/Documents/Analisis/datos_comercio/comercio-exterior/df_spread.csv', row.names = FALSE)


write.csv(paises,  '/Users/karinabartolome/Documents/Analisis/datos_comercio/comercio-exterior/paises.csv', row.names = FALSE)
